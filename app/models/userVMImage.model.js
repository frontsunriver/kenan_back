...
UserVMImage.getAll = (keyword, flag, result) => {
  let query =
    "SELECT user_vm_images.*, vm_image.title, vm_image.description, vm_image.download_url, users.email, users.login_count FROM user_vm_images left join users on users.id = user_vm_images.user_id left join vm_images on vm_images.id = user_vm_images.vm_image_id where 1=1 ";
...
UserVMImage.update = (id, model, result) => {
  sql.query(
    "UPDATE user_vm_images SET user_id = ?, vm_image_id = ?, last_launch_at = ? WHERE id = ?",
    [model.user_id, model.vm_image_id, model.last_launch_at, id],
    (err, res) => {
      if (err) {
        result(null, err);
        return;
